...
public class Philosopher extends Thread {
	...
	public void run() {
		...
		int Sleep = new Random().nextInt(1000);   //philosophers come to table randomly (1-10 seconds)
		...
		table.PutPlate_GUI(id);                       //they put their plates


		try {                                         //they are waiting each other to start
			for (int i = 0; i < N; i++) {
				if (id != i) {
					barriers[i].release();
				}
		...
		table.StartDining_GUI(id);                  //they decided to start eating

		while(true)
		{
			try {                                   //someones wants to think before eating
				Think(id);
			} catch (InterruptedException e) {
				e.printStackTrace();
			}
			...
			try {
				TakeForks(id);                          // just two of them can take forks
			} catch (InterruptedException e) {
				e.printStackTrace();
			}

			table.Eating_GUI(id);                     //and start to eating

			try {
				PutFork(id);                          //they put the forks
			} catch (InterruptedException e) {
				e.printStackTrace();
			}

			table.StopEating_GUI(id);                // the philosophers stop eating and other philosophers starts to eating
		}
	}
	...
	// wh,le truw barrieer ve semafor
	// mutex.release up demekmis  mutex nokta accure down demek..
}	//fonksiyonları tmmlaa
